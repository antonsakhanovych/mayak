...
async fn main() {
    ...
    let stats = Arc::new(StatsReader::new("/data"));

    let rcon_host = std::env::var("RCON_HOST").unwrap_or_else(|_| "server".to_string());
    let rcon_port = std::env::var("RCON_PORT").unwrap_or_else(|_| "25575".to_string());
    let rcon_password = std::env::var("RCON_PASSWORD").expect("RCON_PASSWORD env var not set");
    let rcon = Arc::new(
        RconClient::connect(&format!("{rcon_host}:{rcon_port}"), &rcon_password)
            .await
            .expect("failed to connect to RCON"),
    );
    ...
    let schema = Update::filter_message()
        ...
        .filter(|user: User, allowlist: Arc<Allowlist>| allowlist.contains(user.id))
        .filter_command::<Command>()
        .endpoint(answer);

    Dispatcher::builder(bot, schema)
        .dependencies(dptree::deps![stats, rcon, allowlist])
        .build()
        .dispatch()
        .await;
}
